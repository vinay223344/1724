...
void printStudentsByYear(struct student s[], int n, int year)
{
    printf("Students joined in %d:\n", year);
    for (int i = 0; i < n; i++)
    {
...
void student_data()
{
    ...
    scanf("%d", &year);
    printStudentsByYear(s, 5, year);
    printf("\nSpecify a roll number to get student data: ");
    scanf("%d", &roll);
    printStudentByRoll(s, 5, roll);
}
...
void below_100(struct customer c[])
{
    ...
    for(int i = 0; i < 200; i++)
    {
        if(c[i].account_number != 0 && c[i].balance < 100)
        {
...
void bank_customer()
{
    c[0] = (struct customer){22098, "Mokshagna", 10.50};
    c[1] = (struct customer){27598, "Dhirubhai", 10005.00};
    c[2] = (struct customer){42098, "Jamshetji", 1005.00};
    c[3] = (struct customer){22788, "Elon", 105.00};
    below_100(c)
    transaction(c);
}
   	...
   	scanf("%d %d %d", &current.day, &current.month, &current.year);
    	...
    	for(int i = 0; i < 4; i++)
    	{
        	...
        	if(current.month < emp[i].doj.month ||
          	  (current.month == emp[i].doj.month &&
           	  current.day < emp[i].doj.day))
        	{
            	years--;
        	}
        	if(years >= 3)
...
void sort_books()
{
	...
	for(int j=0; j<i-1; j++)
	{
		for(int k=j+1; k<i; k++)
		{
			if(l[j].accession_number > l[k].accession_number)
			{
				temp = l[j];
				l[j] = l[k];
				l[k] = temp;
	...
	for(int j=0; j<i; j++)
	{
		printf("%d %s\n", l[j].accession_number, l[j].title);
	}
}
void library()
{
	...
	l[0] = (struct library){123,"Let us C","Kanetkar",220,0};
	l[1] = (struct library){345,"C in Depth","Srivastava",400,1};
	l[2] = (struct library){780,"Let us C++","Kanetkar",120,0};
	l[3] = (struct library){190,"Vemana","Vemana",900,0};
...
{
	ih[0] = (struct insurance_holder){0, 0, "LIC", 38};
	ih[1] = (struct insurance_holder){0, 1, "Star", 10};
	ih[2] = (struct insurance_holder){1, 1, "HDFC", 20};
	ih[3] = (struct insurance_holder){0, 1, "ICICI", 100};
	for(int i=0;i<4;i++)
	{
		printf("Name: %s\n", ih[i].policy_name);
...
void uppercase(struct stu st[], int n)
{
	for(int i=0; i<n; i++)
	{
...
void student_uppercase()
{
	...
	st[3] = (struct stu){"Didi", 80, "Kolkata"};
	uppercase(st, 4);
	for(int i=0; i<4; i++)
	{
		printf("Name: %s\n", st[i].name);
...
void calculator()
{
	...
	while(1)
	{
		...
		switch(choice)
		{
			case ADD: printf("Result = %.2f\n", add(a, b)); break;
			case SUB: printf("Result = %.2f\n", sub(a, b)); break;
			case MUL: printf("Result = %.2f\n", mul(a, b)); break;
			case DIV: 
				if(b == 0)
